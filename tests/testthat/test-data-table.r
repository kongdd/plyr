...
test_that("results ordered in order of split variables", {
  d <- data.table(x = c("a","b"), y = c("d","c"), stringsAsFactors = FALSE)

  plyed <- ddply(d, c("x" ,"y"))
  expect_true(is.data.table(plyed))
...
test_that("label variables always preserved", {
  ...
  expect_true(is.data.table(out1))
...
test_that(".id column can be renamed or dropped", {
  ...
  names(l) <- 1:4
  f <- function(l) data.table(sum=sum(unlist(l)))
